<template>
  <div class="menu-page">
    <div class="container">
      <menu-title title="Event Log" @onCloseRequested="onCloseRequested">
        <button
          ...
          @click="markAllRead"
        >
          <i class="fas fa-check"></i> Read All
        </button>
        <button
          :disabled="isLoading"
      ...
      <div class="row pt-2 pb-2">
        <div class="col">
          <input
            ...
            id="showUnread"
          />
          <label class="form-check-label" for="showUnread"> Unread only </label>
        </div>
        <div class="col">
          <select
            :disabled="isLoading"
    ...
    <div
      ...
      v-if="!isLoading && events && !events.length"
    >
      No Events.
    </div>
  </div>
...
const onOpenCarrierDetailRequested = (carrierId: string) =>
  emit("onOpenCarrierDetailRequested", carrierId);
...
const loadEvents = async () => {
  ...
  const response = await listEvents(httpClient)(
    game.value._id,
    page.value,
    pageSize.value,
